...
const Unlimit: FC = () => {
    ...
    const handleCustomOrderIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const getOrders = async (params: string) => {
        const response = await fetch(`/api/proxy?endpoint=/onramp/v1/orders&${params}`, {
            method: "GET",
            redirect: 'follow',
            headers: {
                "access-control-allow-headers": "Accept",
  ...
  const getQuotes = async () => {
    ...
    const queryString = new URLSearchParams(form).toString();
    const response = await fetch(`/api/proxy?endpoint=/onramp/v1/quotes&${queryString}`, {
        method: "GET",
        redirect: 'follow',
        headers: {
            "access-control-allow-headers": "Accept",
